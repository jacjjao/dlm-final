...
|--------|-----------|----------------|
| ASR proxy | `tests/test_asr.py` | Forwarding logic, 503 on unreachable service |
| LLM | `tests/test_llm.py` | Code generation prompt, code-fence stripping, debug parsing |
| Executor | `tests/test_executor.py` | stdout capture, timeout, Docker fallback |
...
│   ├── style.css
│   └── js/
│       ├── main.js         # App entry point & pipeline orchestration
│       ├── api.js          # fetch wrappers for all backend endpoints
│       ├── recorder.js     # MediaRecorder wrapper
│       ├── editor.js       # highlight.js-powered code display
